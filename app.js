...
function loadUserData(uid) {
    ...
    unsubscribeSnapshot = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            if (data.last_executed) {
                ...
                if (lastDate.toDateString() === today.toDateString()) {
                    isDoneToday = true;
                    if (data.status === 'Active') countDoneToday++; // Hanya hitung jika proyek masih aktif
                }
            }
            ...
            let checkInUI = '';
            if (data.task_frequency !== 'One-Time') {
                if (isDoneToday) {
        ...
        // 4. Perbarui Dasbor DOM dengan angka akhir
